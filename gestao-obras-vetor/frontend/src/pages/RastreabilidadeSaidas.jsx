...
export default function RastreabilidadeSaidas() {
  ...
  const [outbound, setOutbound] = useState({ retirado_por_nome: '', quantidade: '', destino: '', elemento_construtivo: '' });
  ...
  const selectReceipt = async (id) => {
    try { setError(''); setNotice(''); setSelected((await getMaterialRecebimento(id)).data); setOutbound({ retirado_por_nome: '', quantidade: '', destino: '', elemento_construtivo: '' }); }
  ...
  const registerOutbound = async () => {
    const isConcrete = selected?.tipo_codigo === 'CONCRETO';
    if (!selected || !outbound.retirado_por_nome.trim() || Number(outbound.quantidade) <= 0 || !outbound.destino.trim() || (isConcrete && !outbound.elemento_construtivo.trim())) {
    ...
    try {
      ...
      setOutbound({ retirado_por_nome: '', quantidade: '', destino: '', elemento_construtivo: '' });
  ...
  const selectedIsConcrete = selected?.tipo_codigo === 'CONCRETO';
    ...
    <main className="container quality-page material-outbound-page">
      ...
      <div className="material-outbound-layout">
        <section className="material-stock-panel">
          ...
          <div className="material-stock-list">{visibleItems.map((item) => <button key={item.id} className={`material-stock-item ${selected?.id === item.id ? 'selected' : ''} ${stockView === 'finalizados' ? 'finished' : ''}`} onClick={() => selectReceipt(item.id)}><div><strong>{item.nome_material}</strong><span>ID do material: {item.codigo} · Lote {item.lote || '—'}</span></div><b>{item.saldo_disponivel} {item.unidade}<small>{stockView === 'estoque' ? 'em estoque' : 'estoque zerado'}</small></b></button>)}</div>
        ...
        <section className="material-outbound-register">
          {!selected ? <div className="material-outbound-placeholder"><Boxes size={34} /><h2>Selecione um material</h2><p>{stockView === 'finalizados' ? 'Escolha um material finalizado para consultar o resumo das saídas.' : 'Escolha um item aprovado com saldo em estoque para registrar a saída.'}</p></div> : stockView === 'finalizados' ? <><div className="material-detail-card-title"><Boxes size={20} /><div><h2>Resumo de saída</h2><p>{selected.nome_material} · ID do material: {selected.codigo} · estoque encerrado</p></div></div><div className="material-stock-summary"><div><span>Liberado</span><strong>{selected.quantidade_aprovada} {selected.unidade}</strong></div><div><span>Total retirado</span><strong>{Math.max(0, Number(selected.quantidade_aprovada) - Number(selected.saldo_disponivel))} {selected.unidade}</strong></div><div><span>Saldo final</span><strong>0 {selected.unidade}</strong></div></div><div className="material-outbound-history"><h3>Histórico de saídas · {selected.codigo}</h3>{selected.aplicacoes?.length ? selected.aplicacoes.map((item) => <div key={item.id}><UserRound size={16} /><div><strong>{item.retirado_por_nome || item.registrado_por_nome || 'Usuário não informado'} retirou {item.quantidade} {item.unidade}</strong><span>Recebimento {selected.codigo} · {item.destino}{item.elemento_construtivo ? ` · ${item.elemento_construtivo}` : ''} · {formatDate(item.aplicado_em)}</span></div></div>) : <p>Nenhuma saída registrada.</p>}</div></> : <><div className="material-detail-card-title"><UserRound size={20} /><div><h2>{selectedIsConcrete ? 'Registrar saída de concreto' : 'Registrar saída'}</h2><p>{selected.nome_material} · ID do material: {selected.codigo} · saldo atual: <strong>{selected.saldo_disponivel} {selected.unidade}</strong></p></div></div><div className="material-stock-summary"><div><span>Liberado</span><strong>{selected.quantidade_aprovada} {selected.unidade}</strong></div><div><span>Já retirado</span><strong>{Math.max(0, Number(selected.quantidade_aprovada) - Number(selected.saldo_disponivel))} {selected.unidade}</strong></div><div><span>Saldo atual</span><strong>{selected.saldo_disponivel} {selected.unidade}</strong></div></div><div className="material-outbound-form material-outbound-form--page"><label className="form-group"><span className="form-label">Quem retirou *</span><input className="form-input" value={outbound.retirado_por_nome} onChange={(event) => setOutbound((current) => ({ ...current, retirado_por_nome: event.target.value }))} placeholder="Nome do usuário ou colaborador" /></label><label className="form-group"><span className="form-label">{selectedIsConcrete ? 'Volume concretado (m³) *' : 'Quantidade retirada *'}</span><input className="form-input" type="number" min="0" max={selected.saldo_disponivel} step="0.001" value={outbound.quantidade} onChange={(event) => setOutbound((current) => ({ ...current, quantidade: event.target.value }))} /></label><label className="form-group"><span className="form-label">Destino / frente de serviço *</span><input className="form-input" value={outbound.destino} onChange={(event) => setOutbound((current) => ({ ...current, destino: event.target.value }))} placeholder="Ex.: Torre A - estrutura" /></label>{selectedIsConcrete && <label className="form-group"><span className="form-label">Elemento / uso efetivo *</span><input className="form-input" value={outbound.elemento_construtivo} onChange={(event) => setOutbound((current) => ({ ...current, elemento_construtivo: event.target.value }))} placeholder="Ex.: Laje do 2º pavimento" /></label>}<Button loading={saving} onClick={registerOutbound}>Confirmar {selectedIsConcrete ? 'concretagem' : 'saída'}</Button></div><div className="material-outbound-history"><h3>Histórico de retiradas · {selected.codigo}</h3>{selected.aplicacoes?.length ? selected.aplicacoes.map((item) => <div key={item.id}><UserRound size={16} /><div><strong>{item.retirado_por_nome || item.registrado_por_nome || 'Usuário não informado'} retirou {item.quantidade} {item.unidade}</strong><span>Recebimento {selected.codigo} · {item.destino}{item.elemento_construtivo ? ` · ${item.elemento_construtivo}` : ''} · {formatDate(item.aplicado_em)}</span></div></div>) : <p>Nenhuma retirada registrada para este material.</p>}</div></>}
        </section>
      </div>
    </main>
  </>;
