...
export default function QualidadeHub() {
  ...
  return <><Navbar/><main className="container quality-page"><div className="page-header"><div><p className="eyebrow">GESTÃO DA QUALIDADE</p><h1>Qualidade</h1><p>Controle de não conformidades e rastreabilidade dos materiais da obra.</p></div></div><div className="quality-kpis"><div className="quality-kpi"><PackageSearch/><div><strong>{stats.total||0}</strong><span>Materiais recebidos</span></div></div><div className="quality-kpi"><ClipboardCheck/><div><strong>{waiting}</strong><span>Aguardando inspeção</span></div></div><div className="quality-kpi"><FileWarning/><div><strong>{rnc||stats.rncs_abertas||0}</strong><span>RNC abertas</span></div></div></div><div className="quality-hub-grid"><button className="quality-hub-card" onClick={()=>navigate(`/projeto/${projetoId}/qualidade/folhas`)}><ClipboardCheck size={28}/><div><h2>Folhas de Verificação</h2><p>Montagem, torque, inspeções por lote e aprovações.</p></div><ArrowRight/></button><button className="quality-hub-card" onClick={()=>navigate(`/projeto/${projetoId}/rnc`)}><FileWarning size={28}/><div><h2>RNC</h2><p>Registre, acompanhe e trate não conformidades.</p></div><ArrowRight/></button><button className="quality-hub-card" onClick={()=>navigate(`/projeto/${projetoId}/rastreabilidade-materiais`)}><PackageSearch size={28}/><div><h2>Rastreabilidade de materiais</h2><p>Recebimento, inspeção, lote, evidências e aplicação.</p></div><ArrowRight/></button></div></main></>;
}
