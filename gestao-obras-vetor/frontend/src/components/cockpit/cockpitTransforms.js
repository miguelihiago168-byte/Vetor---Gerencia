...
export const buildAttentionPoints = ({ cockpit, activities, procurement, assets, curve }) => {
  ...
  if (cockpit?.execution?.days_since_latest >= 3) points.push({ source: 'Execução', priority: 'critical', title: `${cockpit.execution.days_since_latest} dias úteis sem novo RDO`, detail: 'Regra existente do Dashboard', date: cockpit.execution.latest_rdo_activity_date || cockpit.execution.latest_rdo_date, href: 'rdos' });
