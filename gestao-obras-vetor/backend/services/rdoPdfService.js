...
function renderHtml(data) {
  ...
  const assinatura = assinaturaPath && assinaturaPath.startsWith(uploadsRoot + path.sep)
    ? toDataUri(assinaturaPath, 'image/png')
    : null;
  const aprovacaoPath = rdo.status === 'Aprovado' && rdo.aprovado_por_assinatura_png
    ? path.resolve(uploadsDir, String(rdo.aprovado_por_assinatura_png).replace(/\\/g, '/'))
    : null;
  ...
  const fotoCards = fotos.map((foto) => {
    ...
    const legenda = descricao || atividade;
    return `
      <article class="photo-card avoid-break">
        ...
        <div class="photo-caption">
          <strong>${descricao ? 'Descrição' : 'Atividade'}: ${escapeHtml(legenda)}</strong>
          ${descricao && atividade !== '-' ? `<span>Atividade: ${escapeHtml(atividade)}</span>` : ''}
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .doc-title { position: relative; min-height: 34px; margin-bottom: 12px; padding-right: 116px; }
    .doc-title h1 { margin: 0; min-width: 0; text-align: center; }
    .doc-title .status { position: absolute; top: 0; right: 0; white-space: nowrap; }
    .company-brands { display: flex; width: 100%; gap: 10px; margin: 0 0 14px; }
    .company-brand { display: flex; flex: 1 1 0; align-items: center; min-width: 0; min-height: 84px; gap: 10px; padding: 9px; border: 1px solid #b9cddd; border-radius: 7px; background: #f8fbfd; }
    .company-brand img { flex: 0 0 112px; width: 112px; height: 62px; min-width: 0; object-fit: contain; background: white; border-radius: 5px; }
    .company-brand > div { flex: 1 1 0; min-width: 0; }
    ...
    .company-brand strong { display: block; min-width: 0; color: #06263a; font-size: 10.5pt; line-height: 1.25; overflow-wrap: anywhere; word-break: break-word; }
    ...
    .signature-box { min-height: 38mm; display: flex; flex-direction: column; justify-content: flex-end; text-align: center; }
    .signature-image { width: 100%; height: 22mm; object-fit: contain; margin-bottom: 2mm; }
...
async function renderFallbackPdf(data, reason) {
  ...
  const assinaturaFallbackPath = data.rdo.criado_por_assinatura_png
    ? path.resolve(uploadsDir, String(data.rdo.criado_por_assinatura_png).replace(/\\/g, '/'))
    : null;
  const uploadsRoot = path.resolve(uploadsDir);
  if (assinaturaFallbackPath && assinaturaFallbackPath.startsWith(uploadsRoot + path.sep) && fs.existsSync(assinaturaFallbackPath)) {
    doc.moveDown(0.5);
