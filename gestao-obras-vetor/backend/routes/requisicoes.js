...
const montarKanbanRequisicoes = async (where, params) => {
  ...
  const reqs = await allQuery(`
    SELECT
      ...
      (SELECT STRING_AGG(sub_i.descricao, ' • ' ORDER BY sub_i.id) FROM requisicao_itens sub_i WHERE sub_i.requisicao_id = r.id) AS descricao_itens
    FROM requisicoes r
    LEFT JOIN projetos p ON p.id = r.projeto_id
    LEFT JOIN usuarios u ON u.id = r.solicitante_id
  ...
  try {
    ...
    const rows = await allQuery(`
      SELECT
        ...
        (SELECT STRING_AGG(i.descricao, ' • ' ORDER BY i.id) FROM requisicao_itens i WHERE i.requisicao_id = r.id) AS descricao_itens
      FROM requisicoes r
      JOIN projetos p ON p.id = r.projeto_id
      LEFT JOIN usuarios u ON u.id = r.solicitante_id
      ${where}
